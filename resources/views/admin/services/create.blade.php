...
@section('page-title', 'Нова услуга')
...
@section('content')
  ...
  <form action="{{ route('services-admin.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="a-field">
          ...
          <div class="a-lang-field active" data-lang="fr">
            <textarea name="description[fr]" rows="2"
                      placeholder="Кратко описание (Френски)">{{ old('description.fr') }}</textarea>
            @error('description.fr')<div class="a-field-error">{{ $message }}</div>@enderror
          </div>
          <div class="a-lang-field" data-lang="en">
            <textarea name="description[en]" rows="2"
                      placeholder="Кратко описание (Английски)">{{ old('description.en') }}</textarea>
          </div>
        </div>
        ...
        <div class="a-field">
          ...
          <div class="a-lang-field active" data-lang="fr">
            <textarea name="long_description[fr]" rows="5"
                      placeholder="Подробно описание на услугата (Френски)">{{ old('long_description.fr') }}</textarea>
          </div>
          <div class="a-lang-field" data-lang="en">
            <textarea name="long_description[en]" rows="5"
                      placeholder="Подробно описание на услугата (Английски)">{{ old('long_description.en') }}</textarea>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="a-form-card">
      ...
      <div class="a-field">
        <label>Изображение <span style="font-weight:400;font-size:11px;color:#8a96ad">(JPG / PNG / WEBP · max 5 MB)</span></label>
    ...
    <div class="a-form-card">
      ...
      <div class="a-field-row">
        <div class="a-field">
          ...
          <input type="text" name="number" value="{{ old('number') }}" placeholder="01" style="max-width:100px" />
        </div>
        <div class="a-field">
          <label>Ширина в решетката <span style="font-weight:400;font-size:11px;color:#8a96ad">(1–12)</span></label>
          <input type="number" name="col_span" value="{{ old('col_span', 4) }}" min="1" max="12" style="max-width:100px" />
          @error('col_span')<div class="a-field-error">{{ $message }}</div>@enderror
        </div>
        <div class="a-field">
          <label>Ред (сортиране)</label>
          <input type="number" name="sort_order" value="{{ old('sort_order') }}" min="0" style="max-width:100px" />
        </div>
      </div>
      ...
      <div class="a-field" style="display:flex; gap:24px">
        ...
        <label class="a-toggle">
          <input type="checkbox" name="published" value="1" checked {{ old('published', true) ? 'checked' : '' }}>
    ...
    <div class="a-form-footer">
      <button type="submit" class="a-btn a-btn-primary">Създай услуга</button>
...
(function () {
  ...
  var titleFr = document.getElementById('title-fr');
