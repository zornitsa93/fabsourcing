...
@section('welcome')
    <h1 class="title has-text-centered is-family-primary">{{ __('Редактиране на тип') }}</h1>
@endsection
...
@section('content')
    ...
    <section class="section pl-0 pr-0 pt-2">
        <form action="{{ route('setting-types.update',$settingType->id) }}" method="POST">
            @csrf
            @method('PUT')

            <label class="label pt-4">Име</label>
            <section class="section p-0">
                 <input class="input @error('name') is-danger @enderror" type="text" name="name" value="{{ $settingType->name }}">
            </section>
